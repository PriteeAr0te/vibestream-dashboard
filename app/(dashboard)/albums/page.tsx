...
import type { Item } from "../../../hooks/useTrendingSongs";

export default function ForYouPage() {
  const dispatch = useDispatch();

  const likedSongs = useSelector<RootState, Song[]>((state) => state.likedSongs.items);

  const isLiked = (id: string) =>
    likedSongs.some((track) => track.id === id);

  const [limit, setLimit] = useState(10);
  const { data: songs = [], isLoading, isError } =
    useTrendingSongs("albums", limit);
  ...
  const handlePlay = (song: Item) => {
    dispatch(setQueue(songs));
    dispatch(setTrack(song));
  };

  const handleLike = (song: Item) => {
